...
# import csv file
x, sam, chan, cat = mc.load_data('example_data.csv',
                                 sep=';', decimal=',', sort='s')
...
# plot raw data
fig, ax = mc.plot_image_table(x, samples, rows=6, columns=8, titles=sam,
                              xlabel='Time [h]', colors=colors, fwidth=4.5,
                              flength=9, labels=labels,
                              limits=limits,
                              )
savefig('raw_data_plots.pdf', transparent=True, bbox_inches='tight')
close(fig)



# plot single plots
# ax = mc.multiple_axes_plot(x, cat[5], xlabel='Time [h]',
